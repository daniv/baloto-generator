...
class RevanchaResult(Base):
    ...
    __table_args__ = (
        UniqueConstraint("game_id", "game_date", name="uq_revancha_results_game_id_date"),
        CheckConstraint(
            "num_1 >= 1 AND num_5 <= 43 AND num_1 < num_2 AND num_2 < num_3 AND num_3 < num_4 AND num_4 < num_5",
            name="ck_revancha_numbers_ascending",
        ),
        CheckConstraint("balota BETWEEN 1 AND 16", name="ck_revancha_balota"),
    )
